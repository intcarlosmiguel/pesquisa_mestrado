...
int** add_edge(int** viz,int* degree,int** mat,int* shuff,int* edges, int n,int site,double p,int** n_existe, int* existe){
...
int** configuration_model(int** viz,int* degree,int N, double p,int* size,int seed){
    ...
    int *shuff = (int*) malloc(N*sizeof(int));
    int** n_existe = (int **)malloc(0* sizeof(int*));
    ...
    for (int i = 0; i < N; i++){
        ...
        printf("Ego: %d\n",i);
        mat = add_edge(viz,degree,mat,shuff,&edges,N-1,i,p,n_existe,&n);
    }
    for (int i = 0; i < n; i++) free(n_existe[i]);
    free(n_existe);
    free(shuff);
    *size = edges;
    return mat;
